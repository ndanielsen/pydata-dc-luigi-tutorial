...
class cleanCSV(luigi.Task):
    ...
    def run(self):
        df = pd.read_csv(self.input().open('r'), encoding='utf-8-sig')
        df = df[['WARD', 'FEES_PAID', 'PERMIT_ID', 'X', 'Y']]
        with self.output().open('w') as out_file:
            df.to_csv(out_file, sep='\t', encoding='utf-8-sig', index=None)

    def output(self):
        return luigi.LocalTarget('./cleandata/permits-dc-%s.csv' % str(self.param))
...
if __name__ == '__main__':
    luigi.run(['mergeDatasets', '--local-scheduler'])
